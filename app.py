...
password = "sparta"
cxn_str = f"mongodb+srv://test:{password}@cluster0.l8jeyig.mongodb.net/?retryWrites=true&w=majority"
...
def detail(keyword):
    ...
    if type(definitions[0]) is str:
        return redirect (url_for(
            'error',
            word = keyword,
            suggestions = f'{", ".join(definitions)}'
        ))
...
def get_exs():
    word = request.args.get("word")
    example_data = db.examples.find({"word_give" : word})
    examples = []
